...
)
{
    ...
    while (states != 0)
    {
        ...
        for (int32_t i = 0; i < nl; i++) {
            ...
            GRB_TRY (GrB_mxm (symbol_frontier, NULL, NULL, bool_semiring, B[i], frontier, GrB_DESC_RT0)) ;
            GRB_TRY (GrB_mxm (next_frontier, *reachable, GrB_LOR, bool_semiring, symbol_frontier, A[i], GrB_DESC_SC)) ;
        }
